<template>
  <el-dialog v-model="dialogVisible" :title="`分配设备给【${dialogProps.row?.name}】`" width="800px" :destroy-on-close="true">
...
import { getSceneDeviceAssignments, bindDeviceToScene, unbindDeviceFromScene } from '@/api/modules/binding'
...
watch(
  ...
  async (visible) => {
    if (visible) {
      const res = await getSceneDeviceAssignments(dialogProps.value.row.id!)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const sceneId = dialogProps.value.row.id!

    toBind.forEach((deviceId) => promises.push(bindDeviceToScene({ deviceId, sceneId })))
    toUnbind.forEach((deviceId) => promises.push(unbindDeviceFromScene({ deviceId, sceneId })))
